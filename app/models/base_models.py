...
class ProductionLine(Base):
    ...
    lab_tests: Mapped[list[LabTest]] = relationship(
        "LabTest", back_populates="production_line"
    )
    production_sheets: Mapped[list[ProductionSheet]] = relationship(
        "ProductionSheet", back_populates="production_line"
    )
...
class Shift(Base):
    ...
    production_sheets: Mapped[list[ProductionSheet]] = relationship(
        "ProductionSheet", back_populates="shift"
    )
...
class ResinType(Base):
    ...
    name: Mapped[str] = mapped_column(String(50), unique=True)
    production_sheets: Mapped[list[ProductionSheet]] = relationship(
        "ProductionSheet", back_populates="resin_type"
    )
