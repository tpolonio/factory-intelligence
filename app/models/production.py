...
class ProductionSheet(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "panel_length > 0", name="ck_production_sheets_panel_length_positive"
        ),
        CheckConstraint(
            "panel_width > 0", name="ck_production_sheets_panel_width_positive"
        ),
        CheckConstraint(
            "panel_thickness > 0", name="ck_production_sheets_panel_thickness_positive"
        ),
        CheckConstraint(
            "forming_line_speed >= 0",
            name="ck_production_sheets_forming_line_speed_nonnegative",
        ),
        CheckConstraint(
            "press_temperature >= 0",
            name="ck_production_sheets_press_temperature_nonnegative",
        ),
        CheckConstraint(
            "press_pressure >= 0",
            name="ck_production_sheets_press_pressure_nonnegative",
        ),
        CheckConstraint(
            "press_factor >= 0", name="ck_production_sheets_press_factor_nonnegative"
        ),
        CheckConstraint(
            "production_duration >= 0",
            name="ck_production_sheets_production_duration_nonnegative",
        ),
        CheckConstraint(
            "total_downtime >= 0",
            name="ck_production_sheets_total_downtime_nonnegative",
        ),
        CheckConstraint(
            "resin_dosed >= 0", name="ck_production_sheets_resin_dosed_nonnegative"
        ),
        CheckConstraint(
            "paraffin_dosed >= 0",
            name="ck_production_sheets_paraffin_dosed_nonnegative",
        ),
        CheckConstraint(
            "urea_dosed >= 0", name="ck_production_sheets_urea_dosed_nonnegative"
        ),
        CheckConstraint(
        ...
        CheckConstraint(
            "panels_produced >= 0",
            name="ck_production_sheets_panels_produced_nonnegative",
        ),
        CheckConstraint(
            "panels_rejected >= 0",
            name="ck_production_sheets_panels_rejected_nonnegative",
        ),
        CheckConstraint(
    ...
    resin_type: Mapped[ResinType] = relationship(
        "ResinType", back_populates="production_sheets"
    )
    ...
    def rejection_rate(cls):
        return case(
            ...
            else_=(
                cast(func.coalesce(cls.panels_rejected, 0), SaFloat)
                / cast(cls.panels_produced, SaFloat)
            )
            * 100,
        )
