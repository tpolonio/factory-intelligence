...
def create_shift(shift_input: ShiftCreate, db: Session) -> Shift:
...
def list_shifts(
    ...
    if shift_letter:
        statement = statement.where(
            Shift.shift_letter == normalize_shift_letter(shift_letter)
        )

    if press_operator:
        statement = statement.where(
            func.lower(Shift.press_operator)
            == normalize_operator_lookup(press_operator)
        )

    if line_operator:
        statement = statement.where(
            func.lower(Shift.line_operator) == normalize_operator_lookup(line_operator)
        )
